...
if($_POST[name]!=''){
$sql="SELECT uid FROM users WHERE `name` = '$_POST[name]'";
...
if($_POST[typeEquip]=="1"){
	mysql_query("update users set points=points+'$_POST[add1]' WHERE uid = '$dwID[0]' ");
	mysql_query("INSERT INTO console.given_points VALUES (md5($time), '$myname', $serverid, '$dwID[0]', '$_POST[name]', $_POST[add1], '$time')");
}
...
$b=bin2hex('equip,10010105,1|equip,10010115,1|equip,10010125,1|equip,10010135,1|equip,10010145,1|equip,10010155,1');
...
$b=bin2hex('equip,10000805,1|equip,10000815,1|equip,10000825,1|equip,10000835,1|equip,10000845,1|equip,10000855,1');
